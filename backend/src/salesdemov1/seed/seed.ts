...
import { withTransaction, query } from "../db/connection";
import { assertDemoEnvironment, isDemoDryRun } from "../utils/demoGuard";
import { demoLog } from "../utils/demoLog";
import { ensureDemoColumns } from "../utils/schemaGuard";
import { insertDemoUser } from "../models/userModel";
import { insertDemoDriver } from "../models/driverModel";
import { insertDemoTrip, type TripStatusDb } from "../models/tripModel";
import { insertDemoPayment } from "../models/paymentModel";
...
async function ensurePaymentCompat(): Promise<void> {
  await query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS plan_type TEXT DEFAULT 'monthly'`);
...
export async function runSeed(): Promise<void> {
  ...
  if (isDemoDryRun()) {
    demoLog(
      "RESET",
      "Dry run: would ensure schema compat, then insert demo riders/drivers/trips/payments"
    );
    return;
  }
  ...
  const rng = mulberry32(
    ((Date.now() >>> 0) ^ (Math.floor(Math.random() * 0xffffffff) >>> 0) ^ 0x9e3779b9) >>> 0
  );
  const passwordHash = await bcrypt.hash("Demo123!", 10);
  const now = Date.now();

  const vtRows = await query<{ id: string; key: string }>(
    `SELECT id, LOWER(name) AS key FROM vehicle_types WHERE is_active = TRUE`
  );
  const vt: Record<string, string> = {};
  for (const r of vtRows.rows) vt[r.key] = r.id;
  const vtKeys = ["normal", "premium", "vip", "motor"] as const;
  for (const k of vtKeys) {
    if (!vt[k]) throw new Error(`Missing vehicle_types row for "${k}" — run main schema seed first.`);
  }
  ...
  await withTransaction(async (client: pg.PoolClient) => {
    ...
    for (let i = 0; i < RIDERS; i++) {
      ...
      const { id } = await insertDemoUser(client, {
        name: `${fn} ${ln}`,
        email,
        phone,
        passwordHash,
    ...
    type DRow = { userId: string; driverId: string };
    ...
    for (let i = 0; i < DRIVERS; i++) {
      ...
      const { id: userId } = await insertDemoUser(client, {
        name: `${fn} ${ln}`,
        email,
        phone,
        passwordHash,
      ...
      const { id: driverId } = await insertDemoDriver(client, {
        userId,
    ...
    const tripMix: TripStatusDb[] = [];
    for (let i = 0; i < nCompleted; i++) tripMix.push("completed");
    for (let i = 0; i < nCancelled; i++) tripMix.push("cancelled");
    for (let i = 0; i < nActive; i++) tripMix.push(rng() > 0.55 ? "in_progress" : "accepted");
    while (tripMix.length < TRIPS) tripMix.push("pending");
    ...
    function rollPaymentStatus(): "pending" | "approved" | "rejected" {
      const r = rng();
      if (r < 0.62) return "approved";
    ...
    for (let p = 0; p < PAYMENTS; p++) {
      ...
      const expireAt =
        st === "approved" ? new Date(createdAt.getTime() + (days ?? 20) * 86400000) : null;
      const receiptUrl = `https://dummyimage.com/800x200/111/f5b700&text=${encodeURIComponent(`Demo-${p + 1}`)}`;

      await insertDemoPayment(client, {
        driverId: d.driverId,
        amount,
        status: st,
        expireAt,
